# Task 2: Create src/chart/config.ts
...
Create the file at `src/chart/config.ts`. First ensure the directory exists:

```powershell
New-Item -ItemType Directory -Force -Path "src\chart" | Out-Null
```

Then write the file with the complete content below. This extracts types and utility functions from the existing `src/mintwaterfall-chart.ts` (lines 1-316) plus default config values (lines 318-416), refactored to be self-contained:
...
npx tsc --noEmit
```
Expected: may fail due to unresolved imports in other files — OK at this stage, as long as `src/chart/config.ts` itself has no TS errors.
