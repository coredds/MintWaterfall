# Task 5: Create src/chart/chart.ts — Chart Factory

**Files:**
- Create: `src/chart/chart.ts`

This creates the main `waterfallChart()` factory function that composes config, render, and lifecycle modules.

## Prerequisites

The following files already exist in `src/chart/`:
- `config.ts` — exports ChartConfig, defaultConfig, WaterfallChart, ProcessedData, ChartData, MarginConfig, BrushOptions, TooltipConfig, ExportConfig, ZoomConfig, BreakdownConfig, AdvancedColorConfig, ConfidenceBandConfig, MilestoneConfig, calculateIntelligentMargins, getBarWidth, getBarPosition
- `render.ts` — exports drawGrid, drawAxes, drawBars, drawConnectors, drawTrendLine, drawConfidenceBands, drawMilestones
- `lifecycle.ts` — exports prepareData

The following modules still exist at `src/` (will be renamed in Task 8):
- `mintwaterfall-scales.ts` — exports createScaleSystem
- `mintwaterfall-brush.ts` — exports createBrushSystem
- `mintwaterfall-accessibility.ts` — exports createAccessibilitySystem
- `mintwaterfall-tooltip.ts` — exports createTooltipSystem
- `mintwaterfall-export.ts` — exports createExportSystem
- `mintwaterfall-zoom.ts` — exports createZoomSystem
- `mintwaterfall-performance.ts` — exports createPerformanceManager
- `mintwaterfall-shapes.ts` — exports createShapeGenerators

## Steps

- [ ] **Step 1: Create `src/chart/chart.ts`**

The file must import all sub-modules and export the `waterfallChart()` factory function. This replaces the original `waterfallChart()` from `src/mintwaterfall-chart.ts`, combining the chart rendering function (the `chart(selection)` closure) with all getter/setter methods.
...
Fix any TS errors in `src/chart/chart.ts`. Errors from `src/mintwaterfall-chart.ts` (the old file still exists) are expected and OK — those will be deleted in Task 6.

- [ ] **Step 3: Commit**

```bash
git add src/chart/chart.ts && git commit -m "feat: create chart factory in src/chart/chart.ts composing config, render, lifecycle"
```
