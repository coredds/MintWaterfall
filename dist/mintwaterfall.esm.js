...
import { group, index, cross, flatRollup, rollup, ascending, bisector, deviation, variance, median, quantile } from 'd3-array';
...
function calculateIntelligentMargins(processedData, baseMargin, width, height, formatNumber) {
    ...
    const spaceNeededFromTop = Math.max(initialTopMargin - highestLabelPosition + requiredLabelSpace, requiredLabelSpace + safetyBuffer);
...
function createShapeGenerators() {
    ...
    function createConfidenceBand(data, config = {}) {
        const { curve = d3.curveMonotoneX, opacity = 0.3, fillColor = "#95a5a6" } = config;
    ...
    function createSmoothTrendLine(data, config = {}) {
        const { curve = d3.curveMonotoneX, strokeColor = "#e74c3c", strokeWidth = 2, opacity = 0.8 } = config;
...
function drawGrid(container, yScale, config, margins) {
...
function drawAxes(container, xScale, yScale, config, margins) {
    ...
    yAxisGroupEnter.merge(yAxisGroup)
        ...
        .duration(config.duration)
        .ease(config.ease)
        .call(d3.axisLeft(yScale).tickFormat((d) => config.formatNumber(d)));
...
function drawBars(container, processedData, xScale, yScale, config, margins) {
...
function drawStackedBars(barGroups, xScale, yScale, config, margins) {
...
function drawValueLabels(barGroups, xScale, yScale, config, margins) {
    barGroups.each(function (d) {
        const group = d3.select(this);
...
function drawConnectors(container, processedData, xScale, yScale, config) {
    if (config.stacked || processedData.length < 2)
        return;
    ...
    connectorsEnter.merge(connectors)
        ...
        .duration(config.duration)
        .ease(config.ease)
        .delay((d, i) => config.staggeredAnimations ? i * config.staggerDelay : 0)
        .attr("x1", (d) => d.x1)
        .attr("x2", (d) => d.x2)
        .attr("y1", (d) => d.y)
        .attr("y2", (d) => d.y)
...
function drawTrendLine(container, processedData, xScale, yScale, config) {
...
function drawConfidenceBands(container, processedData, xScale, yScale, config) {
    ...
    const confidenceBandData = createWaterfallConfidenceBands(processedData.map(d => ({ label: d.label, value: d.barTotal })), config.confidenceBandConfig.scenarios, xScale, yScale);
...
function drawMilestones(container, processedData, xScale, yScale, config) {
    ...
    const milestoneMarkers = createWaterfallMilestones(config.milestoneConfig.milestones, xScale, yScale);
...
function createScaleSystem() {
    ...
    function createTimeScale(values, options = {}) {
        ...
        if (tickFormat === "auto" && extent[0] && extent[1] && extent[0] instanceof Date && extent[1] instanceof Date) {
            ...
            scale.tickFormat = d3.timeFormat(tickFormat);
        }
        return scale;
    }
    ...
    const chart = function chart(selection) {
        selection.each(function (data) {
            ...
            if (svgNode) {
                ...
                if (w)
                    actualWidth = parseInt(w, 10);
                if (h)
                    actualHeight = parseInt(h, 10);
            }
            ...
            try {
                ...
                const intelligentMargins = calculateIntelligentMargins(processedData, config.margin, actualWidth, actualHeight, config.formatNumber);
                let xScale;
                if (config.scaleType === "auto") {
                    xScale = scaleSystem.createAdaptiveScale(processedData, "x");
                    if (xScale.padding)
                        xScale.padding(config.barPadding);
                }
...
function groupWaterfallData(data, groupBy, valueAccessor, labelAccessor) {
    const grouped = dataProcessor.flatRollupBy(data, (values) => d3.sum(values, valueAccessor), ...groupBy);
