/*!
 * MintWaterfall v0.8.10 - FAST BUILD
...
const defaultConfig = {
    ...
    barPadding: 0.05,
    duration: 750,
    ease: d3__namespace.easeQuadInOut,
    formatNumber: d3__namespace.format(".0f"),
    theme: null,
    enableBrush: false,
    brushOptions: {},
...
function drawAxes(container, xScale, yScale, config, margins) {
    ...
    yAxisGroupEnter.merge(yAxisGroup)
        ...
        .duration(config.duration)
        .ease(config.ease)
        .call(d3__namespace.axisLeft(yScale).tickFormat((d) => config.formatNumber(d)));
...
function drawTrendLine(container, processedData, xScale, yScale, config) {
    ...
    const line = d3__namespace.line()
        .x(d => d.x)
        .y(d => d.y)
        .curve(config.trendLineType === "polynomial" ? d3__namespace.curveCardinal :
        config.trendLineType === "moving-average" ? d3__namespace.curveMonotoneX :
